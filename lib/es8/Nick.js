...
class Nick {
	...
	constructor(options = {}) {
		...
		// local chrome or remote chrome
		if (_.has(options, 'remoteChromeDriver')) {
			if (typeof options.remoteChromeDriver !== 'boolean') {
				throw new TypeError('remoteChromeDriver must be of type boolean')
			}
		} else {
			options.remoteChromeDriver = false
		}

		// Chrome Host
		if (_.has(options, 'chromeHost')) {
			if (typeof options.chromeHost !== 'string') {
				throw new TypeError('chromeHost option must be a number')
			}
		} else {
			options.chromeHost = 'localhost'
		}

		// Chrome Port
		if (_.has(options, 'chromePort')) {
			if (typeof options.chromePort !== 'number') {
				throw new TypeError('chromePort option must be a number')
			}
		} else {
			options.chromePort = 9222
		}
