...
class BrowserDriver {
	...
	_initialize(callback) {
		// just for local, because remote will it running already.
		if (!this._options.remoteChromeDriver) {
			// fix remote port to 9222 when using local chrome driver
			this._options.chromeHost = 'localhost'
			this._options.chromePort = 9222
			this._initialize_local_chrome(callback)
		}
	...
	_initialize_local_chrome(callback) {
		const chromePath = process.env.CHROME_PATH || "google-chrome-beta"
		const childOptions = [
			`--remote-debugging-port=${this._options.chromePort}`,
	...
	startConnection(callback) {
		...
		const checkDebuggerPort = () => {
			setTimeout(() => {
				const socket = net.createConnection(this._options.chromePort)
	...
	_newTabDriver(uniqueTabId, callback) {
		...
		const connectToTab = (cdpTarget) => {
			CDP({ host: this._options.chromeHost, port: this._options.chromePort, target: cdpTarget }, (client) => {
		...
		CDP.List({ host: this._options.chromeHost, port: this._options.chromePort }, (err, tabs) => {
			if (err) {
				...
				if ((uniqueTabId === 1) && (tabs.length === 1) && (tabs[0].url === "about:blank")) {
					...
					CDP.New({ host: this._options.chromeHost, port: this._options.chromePort }, (err, tab) => {
						if (err) {
